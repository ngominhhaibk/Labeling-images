...
class LabelTool():
    ...
    def loadDir(self, dbg = False):
        if not dbg:
            try:
                ...
                return
        #print(os.getcwd())
        if not os.path.isdir('E:/CMC/Annotation_tool_PAR-main/Image/%s' % self.category):
           tkMessageBox.showerror("Error!", message = "The specified dir doesn't exist!")
           return
        # get image list
        self.imageDir = os.path.join(r'E:/CMC/Annotation_tool_PAR-main/Image', '%s' %(self.category))
        ...
        self.total = len(self.imageList)
         ...
         # set up output dir
        if not os.path.exists('E:/CMC/Annotation_tool_PAR-main/Label'):
            os.mkdir('E:/CMC/Annotation_tool_PAR-main/Label')
        self.outDir = os.path.join(r'E:/CMC/Annotation_tool_PAR-main/Label', '%s' %(self.category))
    ...
    def loadImage(self):
        ...
        if os.path.exists(self.labelfilename):
            ...
            if temp_data[1] == "1":
                message_text += "Child\n"
            if temp_data[2] == "1" or temp_data[3] == "1":
                message_text += "Adult\n"
            if temp_data[4] == "1":
                message_text += "Long-hair\n"
            if temp_data[5] == "1":
                message_text += "Glass\n"
            if temp_data[6] == "1":
                message_text += "Hat\n"
            if temp_data[7] == "1":
                message_text += "Short-Sleeves\n"
            if temp_data[8] == "1" or temp_data[9] == "1":
                message_text += "Trousers-Jeans\n"
            if temp_data[10] == "1":
                message_text += "Skirt\n"
            if temp_data[11] == "1":
                message_text += "Short\n"
            if temp_data[12] == "1":
                message_text += "Backpack\n"
            if temp_data[13] == "1" or temp_data[14] == "1":
                message_text += "Other-att\n"  
    ...
    def deleteImage(self):
        self.saveImage()
        shutil.move(self.labelfilename, os.path.join('/home/tuantran/Documents/CMC/data/Duong_Image5/Deleted_Labels',self.labelfilename.split('/')[0]))
        shutil.move(self.imagefilename, os.path.join('/home/tuantran/Documents/CMC/data/Duong_Image5/Deleted_Image',self.imagefilename.split('/')[0]))
        if self.cur < self.total:
            self.imageList.remove(self.imageList[self.cur])
            self.cur += 1
            self.loadImage()
        else:
            tkMessageBox.showerror("Information!", message = "All images annotated")
